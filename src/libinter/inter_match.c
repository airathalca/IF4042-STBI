#ifdef RCSID
static char rcsid[] = "$Header: /home/smart/release/src/libinter/inter_match.c,v 11.0 1992/07/21 18:21:44 chrisb Exp $";
#endif
...
static int help_message(), advanced_help_message();
int show_titles();
...
char *unused;
{
    ...
    PRINT_TRACE (2, print_string, "Trace: entering init_inter_match");
    ...
    PRINT_TRACE (2, print_string, "Trace: leaving init_inter_match");
    return (0);
}

int
inter_match (unused1, unused2, inst)
...
int inst;
{
    ...
    PRINT_TRACE (2, print_string, "Trace: entering inter_match");
    ...
    while (1) {
        ...
        if (UNDEF == (status = command->proc.proc (&is, (char *) NULL)) ||
            ...
            if (command->end_inter)
                PRINT_TRACE (2, print_string, "Trace: leaving inter_match");
            return (status);
        }
...
int
close_inter_match (inst)
int inst;
{
    COMMAND *command;
    PRINT_TRACE (2, print_string, "Trace: entering close_inter_match");
    ...
    PRINT_TRACE (2, print_string, "Trace: leaving close_inter_match");
    return (0);
}

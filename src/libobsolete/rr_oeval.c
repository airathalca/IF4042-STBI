#ifdef RCSID
static char rcsid[] = "$Header: /home/smart/release/src/libobsolete/rr_oeval.c,v 11.0 1992/07/21 18:22:43 chrisb Exp $";
#endif
 ...
 *2 rr_oeval (rr_file, eval, inst)
 *3   char *rr_file;
 *3   OLDSM_EVAL *eval;
 *3   int inst;
 *4 init_rr_oeval (spec, unused)
 *5   "eval.rr_file"
 *5   "eval.rr_file.rmode"
 *5   "eval.trace"
 *4 close_rr_oeval (inst)
 ...
 *8 Call rrvec_oeval for each query in rr_file, and simply average results.
...
static int rrvec_oeval_inst;

int
init_rr_oeval (spec, unused)
SPEC *spec;
char *unused;
{
    ...
    PRINT_TRACE (2, print_string, "Trace: entering init_rr_oeval");

    if (UNDEF == (rrvec_oeval_inst = init_rrvec_oeval (spec, (char *) NULL)))
         return (UNDEF);

    PRINT_TRACE (2, print_string, "Trace: leaving init_rr_oeval");

    return (0);
}

int
rr_oeval (in_rr_file, eval, inst)
char *in_rr_file;
OLDSM_EVAL *eval;
int inst;
{
    ...
    PRINT_TRACE (2, print_string, "Trace: entering rr_oeval");
    ...
    while (1 == read_rr_vec (rr_fd, &rr_vec)) {
        /* Evaluate this query */
        if (UNDEF == rrvec_oeval (&rr_vec, &query_eval, rrvec_oeval_inst))
            return (UNDEF);
    ...
    PRINT_TRACE (2, print_string, "Trace: leaving rr_oeval");
...
int inst;
{
    PRINT_TRACE (2, print_string, "Trace: entering close_rr_oeval");
    if (UNDEF == close_rrvec_oeval (rrvec_oeval_inst))
        return (UNDEF);
    PRINT_TRACE (2, print_string, "Trace: leaving close_rr_oeval");
    return (0);
}
